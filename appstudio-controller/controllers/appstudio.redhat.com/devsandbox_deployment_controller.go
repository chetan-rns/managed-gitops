...
func (r *DevsandboxDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	dt, err := findMatchingDTForSpaceRequest(ctx, r.Client, &spacerequest)
	if err != nil {
	...
	if dt == nil {
		...
		log.Info("DeploymentTarget is created for SpaceRequest", "SpaceRequest.Name", spacerequest.Name,
			"SpaceRequest.Namespace", spacerequest.Namespace)
	}

	log.Info("A DeploymentTarget for the SpaceRequest exists", "DeploymentTarget.Name", dt.Name, "Namespace", dt.Namespace)

	dt.Status.Phase = applicationv1alpha1.DeploymentTargetPhase_Available // set phrase to "Available"
